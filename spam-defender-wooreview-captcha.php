...
Plugin URI: https://github.com/Raisul447/spam-defender-wooreview-captcha
...
Text Domain: spam-defender-wooreview-captcha
*/
...
class SDWC_Review_Captcha {
    ...
    public function add_settings_page() {
        add_options_page(
            esc_html__( 'Woocommerce Review Captcha', 'spam-defender-wooreview-captcha' ),
            esc_html__( 'Woocommerce Review Captcha', 'spam-defender-wooreview-captcha' ),
            'manage_options',
            'sdwc-settings',
    ...
    public function register_settings() {
        ...
        add_settings_field( 'site_key', esc_html__( 'Site Key', 'spam-defender-wooreview-captcha' ), array( $this, 'site_key_field_html' ), 'sdwc-settings', 'sdwc_section' );
        add_settings_field( 'secret_key', esc_html__( 'Secret Key', 'spam-defender-wooreview-captcha' ), array( $this, 'secret_key_field_html' ), 'sdwc-settings', 'sdwc_section' );
    }
    ...
    public function settings_page_html() {
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__( 'Spam Defender – WooCommerce Review Captcha', 'spam-defender-wooreview-captcha' ); ?></h1>
    ...
    public function settings_link( $links ) {
        $settings_link = '<a href="options-general.php?page=sdwc-settings">' . esc_html__( 'Settings', 'spam-defender-wooreview-captcha' ) . '</a>';
    ...
    public function verify_recaptcha_server_side( $commentdata ) {
        ...
        if ( ! isset( $_POST['sdwc_recaptcha_nonce'] ) || 
            ...
            $this->redirect_with_error( __( 'Security check failed. Please reload the page and try again.', 'spam-defender-wooreview-captcha' ) );
        }
    
        // Check response
        if ( empty( $_POST['g-recaptcha-response'] ) ) {
            $this->redirect_with_error( __( 'Please complete the reCAPTCHA before submitting your review.', 'spam-defender-wooreview-captcha' ) );
        }
        ...
        if ( empty( $result ) || empty( $result->success ) || $result->success !== true ) {
            $this->redirect_with_error( __( 'reCAPTCHA verification failed. Please try again.', 'spam-defender-wooreview-captcha' ) );
        }
